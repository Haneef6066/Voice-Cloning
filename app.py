...
# Lazy load XTTS model (GPU disabled by default; enable if you have CUDA)
_tts_instance = None

def get_tts():
    """Lazy load the TTS model to avoid memory issues at startup."""
    global _tts_instance
    if _tts_instance is None:
        print("Loading TTS model...")
        _tts_instance = TTS("tts_models/multilingual/multi-dataset/xtts_v2", progress_bar=False, gpu=False)
...
def create_clone():
    ...
    sample_text = request.form.get('sample_text') or 'This is a cloned voice sample.'
    language = request.form.get('language') or 'en'

    # sanitize the user-provided name to remove slashes etc.
    safe_name = secure_filename(name.replace('/', '_').replace('\\', '_'))
    if not safe_name:
        safe_name = f"voice_{uuid.uuid4().hex[:6]}"

    # save uploaded file (preserve extension)
    fname = secure_filename(file.filename or (safe_name + '.wav'))
    base, ext = os.path.splitext(fname)
    if not ext:
        ext = '.wav'
    vid = uuid.uuid4().hex[:8]
    saved_name = f"{safe_name}_{vid}{ext}"
    ...
    if ext.lower() != '.wav':
        try:
            wav_path = os.path.join(CLONED_FOLDER, f"{safe_name}_{vid}.wav")
    ...
    # synthesize a short sample and save it as sample_<id>.wav (optional, may fail due to memory)
    sample_name = f"sample_{vid_key}.wav"
    sample_path = os.path.join(CLONED_FOLDER, sample_name)
    sample_generated = False
    try:
        t_start = time.time()
        get_tts().tts_to_file(text=sample_text, file_path=sample_path, speaker_wav=saved_path, language=language)
        t_end = time.time()
        ...
        try:
            ...
            rtf = proc_time / dur_sec if dur_sec > 0 else 0
            print(f" > Processing time: {proc_time:.4f}")
            print(f" > Real-time factor: {rtf:.4f}")
        except Exception:
            print(f" > Processing time: {proc_time:.4f}")
        sample_generated = True
    except Exception as e:
        app.logger.warning(f'Failed to synthesize sample for new clone (this is optional): {e}')
        sample_name = None  # No sample available

    return jsonify({'id': vid_key, 'name': name, 'file': saved_name, 'sample': sample_name, 'sample_generated': sample_generated})
...
def speak():
    ...
    repetition_penalty = float(data.get('repetition_penalty', 5.0))
